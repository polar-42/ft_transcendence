...
function FP_Init()
{
	...
	renderer.setSize(720, 480); //change define

	//Add threejs canvas to page
	...
	BoatList = [
		{ name : 'Carrier', x : 0, y : 0, startX : 700, startY : 150, ArrayX : 0, ArrayY : 0, size : 5, horizontal : true, isDragging : false },
		{ name : 'BattleShip', x : 0, y : 0, startX : 700, startY : 250, ArrayX : 8, ArrayY : 0, size : 4, horizontal : true, isDragging : false },
		{ name : 'Destroyer', x : 0, y : 0, startX : 700, startY : 350, ArrayX : 4, ArrayY : 2, size : 3, horizontal : true, isDragging : false },
		{ name : 'Submarine', x : 0, y : 0, startX : 700, startY : 450, ArrayX : 0, ArrayY : 2, size : 3, horizontal : true, isDragging : false },
		{ placed : false, name : 'PatrolBoat', x : 0, y : 0, startX : 700, startY : 550, ArrayX : 8, ArrayY : 2, size : 2, horizontal : false, isDragging : false },
	]
...
const box_color = new THREE.MeshBasicMaterial({ color: 0xffffff });
function CreateABox(x, y)
{
	let Box = {
		...
		ArrayPosY : y,
		status: 0,
		self: new THREE.Mesh( geometry, box_color )
	}
	Box.self.position.x = x
	Box.self.position.y = y
	return Box
}
...
function drawGrid(drawPart)
{
	BoardCases.forEach(element => {
		...
		{
			element.self.material.color.setHex(0x00FF00)
		}
		if(drawPart == 0 || drawPart == 2)
		{
			...
			{
				if (element.status == 1)
					element.self.material.color.setHex(0x0000FF)
				else if (element.status == -1)
					element.self.material.color.setHex(0xA0A003)
			}
			else if (SP_selected != undefined && SP_selected == element)
				element.self.material.color.setHex(0x555555)
			else if (SP_hovered != undefined && SP_hovered == element)
				element.self.material.color.setHex(0xFF0000)
			else
				element.self.material.color.setHex(0xA0FF03)
		}
	})
}
